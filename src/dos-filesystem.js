...
export default class DosFileSystem {
  ...
  static async isFileExist(path) {
    try {
      ...
      // console.log({ st, isFile: st.isFile(), isDir: st.isDirectory() })
  ...
  static async readText(path, encode = 'utf8') {
    if (!(await DosFileSystem.isFileExist(path))) return Promise.reject('')
    ...
    return new Promise((resolve, reject) => {
      ...
      fs.readFile(path, { flag: 'r' }, (err, data) => {
        ...
        if (
          ['sjis', 'shift-jis', 'shiftjis'].indexOf(encode.toLowerCase()) >= 0
        ) {
  ...
  static async move(fromPath, toPath, isCopy = false) {
    ...
    if (
      toPath.lastIndexOf('.') > toPath.replaceAll('\\', '/').lastIndexOf('/')
    ) {
      await DosFileSystem.createDirectory(
        toPath.replaceAll('\\', '/').deleteFromEnd('/')
      )
    } else {
